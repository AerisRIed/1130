...
my ($rtl_search_path,$rule_level) = ("","std");
...
my $file_prefix = "litmus";
if (-e $final_rule_settings) {
  my $loc_basename = basename($final_rule_settings,".txt");
  $loc_basename =~ s/(\S+)_final_rule_settings/$1/;
  $file_prefix = $loc_basename;
}
my $changed_rule_settings = "${workdir}/modified_rules/${file_prefix}_changed_rules.txt";
my $suppressed_rule_settings = "${workdir}/modified_rules/${file_prefix}_suppressed_rules.txt";
