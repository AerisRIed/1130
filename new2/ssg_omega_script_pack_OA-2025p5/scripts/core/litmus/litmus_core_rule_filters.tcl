...
if {($prl_rule_level eq "min") || ($prl_rule_level eq "std")} {
  set cetfe_demote_to_note [list]
  if {$rtl_flow == 1} {
    # Unbuffered clock tree nets connected to output ports
    lappend cetfe_demote_to_note "CONST_CLK_DEF7"
  }
  # Undefined set_max_capacitance on output or inout port
  lappend cetfe_demote_to_note "CONST_IO_DRC2"
  # Clock is not set as set_dont_touch_network
  lappend cetfe_demote_to_note "CONST_HFN_CHK1a"
  # Set/reset is not set as set_dont_touch_network
  lappend cetfe_demote_to_note "CONST_HFN_CHK1b"
  # Undefined clock network latency for real and generated clocks
  lappend cetfe_demote_to_note "CONST_CLK_LAT1"
  # Undefined clock source latency for generated clocks
  lappend cetfe_demote_to_note "CONST_CLK_LAT6"
  # Undefined or zero source clock latency for virtual clock
  lappend cetfe_demote_to_note "CONST_CLK_LAT12"
  # Clock network latency is missing
  lappend cetfe_demote_to_note "CONST_CLK_LAT13"
  # False path does not have both -from and -to
  lappend cetfe_demote_to_note "CONST_LINT_FLP1"
  # Multi-cycle path does not have both -from and -to
  lappend cetfe_demote_to_note "CONST_LINT_MCP1"
  # Exception is specified with clock only
  lappend cetfe_demote_to_note "CONST_LINT_EXC2"

  foreach cdtn $cetfe_demote_to_note {
    if {[llength [get_db rule_instances $cdtn]] != 0} {
      set loc_current_severity [get_db [get_db rule_instances $cdtn] .${var_prefix}_severity]
      puts [format "%s%s%s%s%s%s" "In" "fo    : Demoting severity of rule " $cdtn " from default of " $loc_current_severity " to Note."]
      set_message -id $cdtn -severity note
    } else {
      puts [format "%s%s%s%s" "Warn" "ing : The rule " $cdtn " does not exist and hence cannot be demoted to severity Note."]
    }
  }

  # Suppress all messages with severity "Note"
