...
# from the jasper/common/ area only for the CDC App.	#
#########################################################

if {$prl_debug_mode == 1} {
  puts [format "%s%s" "In" "fo    : enabling CDC debug settings"]
  set_message -debug CDC REGRESSION
  set_message -debug GRAPH REGRESSION
}
