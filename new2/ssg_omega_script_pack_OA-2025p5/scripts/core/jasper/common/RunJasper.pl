...
chomp($datestamp);
my %app_hash;
$app_hash{superlint} = {supported => 1, min_stage => 1, max_stage => 5, selected => 0, has_hier_flow => 1, has_debug_mode => 1, app_tag => "-superlint", output_dirs => [qw(db csv html report collateral)]};
$app_hash{cdc} =       {supported => 1, min_stage => 1, max_stage => 6, selected => 0, has_hier_flow => 1, has_debug_mode => 1, app_tag => "-cdc",       output_dirs => [qw(db csv html report collateral)]};
$app_hash{mcp} =       {supported => 1, min_stage => 1, max_stage => 4, selected => 0, has_hier_flow => 0, has_debug_mode => 0, app_tag => "-mcp",       output_dirs => [qw(db report collateral)]};
$app_hash{xprop} =     {supported => 1, min_stage => 1, max_stage => 4, selected => 0, has_hier_flow => 0, has_debug_mode => 0, app_tag => "-xprop",     output_dirs => [qw(db csv html report collateral)]};
$app_hash{sec} =       {supported => 0, min_stage => 1, max_stage => 4, selected => 0, has_hier_flow => 0, has_debug_mode => 0, app_tag => "-sec",       output_dirs => [qw(db report collateral)]};
$app_hash{lpv} =       {supported => 0, min_stage => 1, max_stage => 7, selected => 0, has_hier_flow => 0, has_debug_mode => 0, app_tag => "-lpv",       output_dirs => [qw(db report collateral)]};
my ($run_stage,$gen_reports,$add_perl_vars,$envload) = (0,0,1,0);
my ($workdir,$jg_app,$ten_dpc,$invoke_gui,$bsub_args) = ("","",0,0,"");
my ($rtl_flow,$hier_flow,$threadcount,$dirclean,$debug_mode) = (1,0,1,0,0);
my ($bsubCmd,$jgExe,$jgCmd,$runlog,$guilog,$parselog,$rule_summary_report) = ("","","","","","","");
my @dirs_to_create = ();
my $rule_level;
...
Jasper Apps can only be run one at a time. An App from the set
{superlint,cdc,mcp,xprop} must be specified, as no default is set.
...
-app <string>	Specify which Jasper App to run. This is mandatory.
		Allowed value is one of {superlint,cdc,mcp,xprop}
...
-stage <int>	If using Jasper interactively via the "-gui" option, it is
		...
		Stage # |  Superlint  |      CDC      |     MCP     |    XPROP    |
		--------|-------------|---------------|-------------|-------------|
		   1    | load design | load design   | load design | load design |
		   2    | configure   | domain        | generate    | setup X     |
		   3    | extract     | scheme        | prove       | prove       |
		   4    | prove       | reset         | full run    | full run    |
		   5    | full run    | metastability |             |             |
		   6    |             | full run      |             |             |
...
-debug		If specified, Jasper will provide additional logging information for
		supported Apps to assist in debugging certain issues. The supported set
		of Apps is: {superlint,cdc}. Only use this feature if requested to do
		so as it increases overall runtime.
...
die "Error: The $jg_app App does not have a debug mode.\n" if (($debug_mode == 1) && ($app_hash{$jg_app}{has_debug_mode} == 0));
...
if (defined $app_hash{$jg_app}) {
  ...
  $guilog = "${workdir}/jgproject/sessionLogs/session_0/jg_session_0.log";
...
if ($add_perl_vars) {
  ...
  foreach my $app_name (sort keys %app_hash) {
    print $out_fp "set prl_run_${app_name} $app_hash{$app_name}{selected}\n";
  }
...
CoreUtils::DualPrint($tee_fp,"Info: debug mode has been enabled for the $jg_app App.") if ($debug_mode == 1);
...
} elsif ($jg_app eq "xprop") {
  $rule_summary_report = <${workdir}/report/*_total_property_summary.txt>;
}
$parselog = ($invoke_gui) ? $guilog : $runlog;
&JasperRunCheck($parselog,$rule_summary_report,$gen_reports,$jg_app,$run_stage,$tee_fp);
close($tee_fp);
...
sub JasperSummaryFileCheck {
  ...
  my ($loc_sdc_good,$loc_mcp_sdc_good_count,$loc_xprop_cat,$loc_xprop_cex) = (0,0,"",0);
  my ($loc_failing_message,$loc_passing_message) = ("","");
  if ($loc_app eq "mcp") {
    $loc_failing_message = "MCP/FP SDC commands that are not successfully proven";
    $loc_passing_message = "with no unwaived failing checks";
  } elsif ($loc_app eq "xprop") {
    $loc_failing_message = "X-propagation counterexamples for the selected target endpoints";
    $loc_passing_message = "with no unwaived X-propagation counterexamples";
  } else {
    $loc_failing_message = "unwaived error/warning/info messages";
    $loc_passing_message = "with no unwaived failing checks";
  }
  if (!(-r $app_summary_report)) {
    ...
    close(SUMFILE);
    if (($loc_app eq "mcp") && ($loc_mcp_sdc_problem_count > 0)) {
      ...
      $loc_fail_count++;
    } elsif (($loc_app eq "mcp") && ($loc_mcp_sdc_good_count > 0) && ($loc_mcp_proven_count == 0) && ($total_problem_count == 0)) {
      ...
      CoreUtils::DualPrint($loc_fp,"Error: Jasper $loc_app validation shows a total of $total_problem_count ${loc_failing_message}. Review the following file: ${app_summary_report}.");
      $loc_fail_count++;
    } else {
      CoreUtils::DualPrint($loc_fp,"Info: Jasper $loc_app completed ${loc_passing_message}; the validation was successful.");
    }
  }
