...
###############################################################################
#
# Filename  : cdrlf_ll_wrapper.scan_shift.tcl
# Version   : $Revision$
# Date/Time : $Date$
#
# Author      : John Paul Saavedra
# Modified by : 
# Abstract    : Scan shift timing constraints for cdrlf_ll_wrapper.
#
###############################################################################
# Change Log
# $Log$
...
if { ![info exists ::DRIVING_CELL] } {
   set ::DRIVING_CELL BUFFD4BWP*DSVT
   set ::DRIVING_PIN Z
}
...
set_case_analysis 1 [get_ports { rxda_cdrlf_scanen }]
set_case_analysis 1 [get_ports { rxda_cdrlf_scanen_cg }]
...
# Scan clock is at 200Mhz
create_clock -name SHIFT_CLK \
    -period ${CLOCK_PERIOD} \
...
# Input delays
set_input_delay  0.00                     -max -clock [get_clocks { IO_ASYNC }]  [remove_from_collection [all_inputs -no_clocks]  [get_ports { rxda_cdrlf_scanin[*] clock rxda_cdrlf_scanclk }]]
set_input_delay  ${CLOCK_TWOTHIRD_PERIOD} -max -clock [get_clocks { SHIFT_CLK }] [get_ports { rxda_cdrlf_scanin[*] }]

# Output delays
set_output_delay 0.00                     -max -clock [get_clocks { IO_ASYNC }]  [remove_from_collection [all_outputs] [get_ports { rxda_cdrlf_scanout[*] }]]
set_output_delay ${CLOCK_ONETHIRD_PERIOD} -max -clock [get_clocks { SHIFT_CLK }] [get_ports { rxda_cdrlf_scanout[*] }]

if { [info exists ::do_libgen] && $::do_libgen } {
   set_input_delay  0.0 -min -clock [get_clocks { SHIFT_CLK }] [get_ports { rxda_cdrlf_scanin[*] }]
   set_output_delay 0.0 -min -clock [get_clocks { SHIFT_CLK }] [get_ports { rxda_cdrlf_scanout[*] }]
}
