...
# Modified by : 
# Abstract    : stuck-at capture timing constraints for cdrlf_wrapper.
#
###############################################################################
# Change Log
# $Log$
...
# main scan clock running at 500Mhz 
create_clock -name SCAP_CLK \
    -period ${CLOCK_PERIOD} \
...
# Input delays
set_input_delay  0.00                     -max -clock [get_clocks { IO_ASYNC }]                                       [get_ports { rxda_cdrlf_scanin[*] rxda_cdrlf_reset_n phase_*}]
set_input_delay  ${CLOCK_TWOTHIRD_PERIOD} -max -clock [get_clocks { SCAP_CLK }] [remove_from_collection [all_inputs -no_clocks]  [get_ports { rxda_cdrlf_scanin[*] rxda_cdrlf_reset_n phase_*}]]

# Output delays
set_output_delay 0.00                     -max -clock [get_clocks { IO_ASYNC }]                                       [get_ports { rxda_cdrlf_scanout[*] }]
#set_output_delay ${CLOCK_TWOTHIRD_PERIOD} -max -clock [get_clocks { SCAP_CLK }] [remove_from_collection [all_outputs] [get_ports { rxda_cdrlf_scanout[*] iq_pi_ctrl[*] e_pi_ctrl[*]}]]

if { [info exists ::do_libgen] && $::do_libgen } {
   set_input_delay  0.00 -min -clock [get_clocks { SCAP_CLK }] [remove_from_collection [all_inputs]  [get_ports { rxda_cdrlf_scanin[*] rxda_cdrlf_reset_n phase_*}]]
   #set_output_delay 0.00 -min -clock [get_clocks { SCAP_CLK }] [remove_from_collection [all_outputs] [get_ports { rxda_cdrlf_scanout[*] iq_pi_ctrl[*] e_pi_ctrl[*]}]]
}
