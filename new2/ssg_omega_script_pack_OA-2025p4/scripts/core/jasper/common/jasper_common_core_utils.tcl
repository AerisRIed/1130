...
proc CETFE_jg_elaborate_design {} {
  ...
  if {[llength $bbox_module_list] > 0} {
    eval "elaborate -bbox_m {$bbox_module_list} -top $top_design $additional_args"
  } else {
    eval "elaborate -top $top_design $additional_args"
  }
  liberty -list
}
...
proc CETFE_jg_load_sdc {} {
  ...
  } elseif {$prl_run_superlint == 0} {
    puts [format "%s%s" "Err" "or   : no SDC file was defined via the golden_sdc_file variable"]
    exit 1
  }
}
...
proc CETFE_jg_mcp_report_collateral {} {
  ...
  check_timing_exception -export -force -to [file join $prl_stamped_base_dir collateral ${top_design}_to_excelium_tcv.json]
}
