...
proc CETFE_manipulate_omega_user_vars {{target_tool ""}} {
  ...
  if {[regexp {^(jasper|clp)$} $target_tool] == 1} {
  ...
  # Manipulate golden_name_map_file and hier_jdb_mod_app:
  ...
  # Manipulate mmmc_file, golden_db_file, and stimulus_file_list:
...
proc CETFE_validate_omega_user_vars {{target_tool ""}} {
  ...
  set test_for_boolean [list use_mmmc rtl_flow insert_clock_gating]
  set test_for_integer [list tool_thread_count structure_threshold depth_threshold \
			     ...
			     cts_branch_fanout cts_leaf_fanout]
