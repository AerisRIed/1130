...
if (`grep -c 'auto_master_lnx_sjtencad' /etc/auto.master` || `grep -c 'auto_master_lnx_tensilica' /etc/auto.master`) then
  if (-x /cad/scripts/genus) then
    echo "Detected Tensilica user with /cad/scripts access: setting tool versions directly for OA-2025.3\n"
...
if ($RUN_MODULE_LOAD == 1) then
  if ((`command -v module` != "") || (`alias module` != "")) then
    echo "The module command is already defined or aliased - proceeding to module loading. Note that if modules fail to load,"
    echo "try to unalias the existing module command and run this script again prior to contacting Omega support\n"
  else
    echo "No module command found by default; setting up access to modules instead\n"
    if ($?tcsh) then
  ...
  echo "Loading modules for required tool versions of OA-2025.3:\n"
